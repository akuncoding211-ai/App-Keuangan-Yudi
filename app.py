...
def get_google_sheet():
    try:
        # Ambil credentials langsung dari secrets (TOML format)
        creds_dict = st.secrets["gcp_service_account"]
        
        # Gunakan google-auth yang lebih baru
        ...
        st.error("❌ Secret 'gcp_service_account' tidak ditemukan. Periksa Streamlit Secrets.")
        return None
    except gspread.exceptions.SpreadsheetNotFound:
        st.error("❌ Spreadsheet tidak ditemukan. Cek ID spreadsheet Anda.")
        return None
...
if sheet is None:
    st.warning("⚠️ Tidak dapat terhubung ke Google Sheets. Periksa konfigurasi.")
else:
    try:
        # 1. Dashboard
        if choice == "📊 Dashboard":
        ...
        st.info("Tip: Pastikan semua sheet (Transaksi, Tabungan, Budget, Kewajiban) ada di spreadsheet Anda.")
